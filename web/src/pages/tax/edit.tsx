...
export default function EditTax() {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.put(`${base_url}/api/v1/tax/${id}`, values)
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        <FormTax form={form} onSave={onSubmit} onCancel={onCancel} loading={isLoading} asEdit={id ? true : false} />
      </Card>
    </div>
  )
}
